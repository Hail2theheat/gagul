-- =====================================================
-- FIX: weekly_winners.user_id should be winner_user_id
...
BEGIN
  ...
  -- Get winner info (column is winner_user_id, not user_id)
  SELECT ww.winner_user_id, ww.has_chosen, ww.chosen_prompt_id, ww.custom_prompt_content,
         pr.username, pr.avatar_config
  INTO v_winner
  FROM weekly_winners ww
  JOIN profiles pr ON pr.id = ww.winner_user_id
  ...
  v_result := jsonb_build_object(
    ...
    'winner', CASE WHEN v_winner.winner_user_id IS NOT NULL THEN jsonb_build_object(
