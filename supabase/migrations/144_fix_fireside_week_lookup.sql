-- Fix get_fireside_data to find the most recent week_of from actual data
-- instead of computing it with date_trunc (which doesn't match our week_of values)
...
BEGIN
  ...
  IF p_week_of IS NOT NULL THEN
    v_week := p_week_of;
  ELSE
    SELECT gp.week_of INTO v_week
    FROM group_prompts gp
    WHERE gp.group_id = p_group_id
    ORDER BY gp.week_of DESC
    LIMIT 1;
  ...
  -- Get prompts with responses (exclude meme + photo_caption from regular list)
  ...
  FROM (
    ...
    WHERE gp.group_id = p_group_id
      ...
      AND p.type NOT IN ('meme_upload', 'meme_caption', 'photo_caption')
